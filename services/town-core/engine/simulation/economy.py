...
def set_merchant_prices(db: Session, npc_id: int) -> dict:
    ...
    """
    from engine.models import NPC, Building, Resource, WorldState
    import json
...
def detect_monopoly(db: Session) -> List[str]:
    ...
    """
    from engine.models import Resource, NPC, Event
...
def process_black_market(db: Session) -> int:
    ...
    from engine.models import Crime, Resource, Building, WorldState
    import random
...
def detect_economic_bubble(db: Session) -> list[str]:
    ...
    for (resource_name,) in resource_names:
        ...
        prices = (
            ...
            .filter(PriceHistory.resource_name == resource_name)
...
def calculate_wage_disparity(db: Session) -> float:
    ...
    working_npcs = db.query(NPC).filter(NPC.is_dead == 0, NPC.work_building_id.is_not(None)).all()
...
def allocate_budget(db: Session) -> dict:
    ...
    defense_event = Event(
        event_type="defense_budget",
        description="Defense budget allocated",
...
def check_food_scarcity(db: Session) -> bool:
    ...
    from engine.models import Resource, Event, WorldState
...
def update_market_prices(db: Session) -> dict:
    ...
    from engine.models import Resource, WorldState, NPC
