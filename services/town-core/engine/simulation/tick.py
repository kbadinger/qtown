...
def process_tick(db: Session) -> None:
    ...
    npcs = db.query(NPC).filter(NPC.is_dead == 0, NPC.traveling == 0).all()
    for npc in npcs:
    ...
    # v2: Check for timed-out NPC travels
    timed_out = check_travel_timeouts(db, world_state.tick)
    if timed_out:
        import logging
        logging.getLogger("town-core.tick").info(
            "Travel timeouts: %d NPCs returned", len(timed_out))
...
def calculate_approval_rating(db: Session) -> dict | None:
    ...
    latest_election = (
        db.query(Election).filter(Election.winner_npc_id.is_not(None)).order_by(Election.id.desc()).first()
    )
...
def allocate_town_budget(db: Session) -> dict:
    ...
    event = Event(
        ...
        description=(
            f'Allocated {total_gold} gold: infrastructure={infrastructure}, '
            f'defense={defense}, welfare={welfare}, events={events_allocation}'
        ),
        tick=current_tick,
        severity='info'
    )
...
def generate_daily_summary(db: Session) -> dict:
    ...
    # Create Event
    description = (
        f"Day {current_day}: {living_count} alive, {dead_count} dead, {int(total_gold)}g, "
        f"{unresolved_crimes} crimes, avg happiness {avg_happiness:.0f}"
    )
    event = Event(
